...
/**
 * Courier-side order detail — shows the parcels inside an ASSIGNMENT order
 * given to this courier. Read-only header; status updates still happen from
...
export default async function CourierOrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const order = await prisma.order.findUnique({
    ...
    include: {
      assignmentDeliveries: {
        ...
        include: { order: { select: { id: true, orderNumber: true, notes: true } } },
      },
    },
  })
  if (!order) notFound()
  // Hard guard: a courier can only see their own assignment orders.
  if (baseSession.user?.role === 'COURIER' && order.courierId !== courierId) {
  ...
  return (
    <Shell
      ...
      breadcrumb={{ href: '/courier/orders', label: t('order_back_to_list') }}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 bg-[var(--color-card)] rounded-2xl shadow-sm border border-[var(--color-border)] overflow-hidden">
          <div className="px-6 py-3 border-b border-[var(--color-border)]">
            <p className="text-xs font-semibold text-[var(--color-text-faint)] uppercase tracking-wide">
              {t('order_parcels_section')} ({order.assignmentDeliveries.length})
            </p>
          </div>
          <ul className="divide-y divide-[var(--color-border)]">
            {order.assignmentDeliveries.map((d) => (
              <li key={d.id}>
                <Link href={`/courier/deliveries/${d.id}`} className="flex items-center gap-3 px-6 py-3 hover:bg-[var(--color-card-hover)] transition-colors">
