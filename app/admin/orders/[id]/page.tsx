...
/**
 * Admin Order detail — header summary + every parcel that came in on this
 * upload, with current status. Each parcel links through to its own detail
 * page for assigning a courier, viewing history etc.
 */
export default async function AdminOrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <Shell
      ...
      subtitle={`${order.company.name} · ${new Date(order.createdAt).toLocaleString()}`}
    >
          ...
          <div className="bg-[var(--color-card)] rounded-2xl shadow-sm border border-[var(--color-border)] p-5">
            ...
            <dl className="text-sm space-y-2">
              ...
              <div className="flex justify-between gap-3">
                ...
                <dd className="font-semibold text-[var(--color-text-strong)] text-right truncate">{order.company.name}</dd>
              </div>
              <div className="flex justify-between gap-3">
