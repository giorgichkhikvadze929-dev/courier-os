...
export default function BulkPanel({
  ...
  return (
    <>
      ...
      {/* ── lg+ screens: persistent right-side cart drawer ──────────────
          Mirrors the mockup's two stacked cards:
            · Selected Items (N) — parcel list + clear all
            · Assign Courier / Verify-Deny — action panel + big CTA */}
      {selected.size > 0 && (
        <aside className="hidden lg:flex flex-col fixed right-4 top-20 bottom-4 w-80 xl:w-96 z-20 gap-3 pointer-events-auto">
          {/* Card 1 — Selected Items */}
          <div className="bg-[var(--color-card)] rounded-2xl shadow-lg border border-[var(--color-border)] flex flex-col overflow-hidden flex-1 min-h-0">
            ...
            <ul className="divide-y divide-[var(--color-border)] flex-1 overflow-y-auto">
              {selectedRows.map((d) => (
                <li key={d.id} className="px-4 py-3 flex items-start gap-3 hover:bg-[var(--color-card-hover)] transition-colors">
                  ...
                  <button
                    type="button"
                    onClick={() => toggle(d.id)}
                    className="text-[var(--color-text-faint)] hover:text-red-600 dark:hover:text-red-300 text-lg leading-none flex-shrink-0"
                    title={t('btn_remove')}
                  >×</button>
                </li>
              ))}
            </ul>
            <div className="px-4 py-3 border-t border-[var(--color-border)] flex-shrink-0 bg-[var(--color-card-hover)]/40">
          ...
          {/* Card 2 — Action panel (Assign Courier OR Verify/Deny) */}
          <div className="bg-[var(--color-card)] rounded-2xl shadow-lg border border-[var(--color-border)] p-4 flex-shrink-0">
            {selectionMode === 'assign' && (
              <>
                <p className="text-xs font-semibold uppercase tracking-wider text-[var(--color-text-faint)] mb-2">{t('bulk_assign_courier_title')}</p>
                <select
                  value={chosenCourier}
                  onChange={(e) => setChosenCourier(e.target.value)}
                  className="w-full border border-[var(--color-border-strong)] rounded-xl px-3 py-2.5 text-sm bg-[var(--color-card)] text-[var(--color-text-strong)] mb-3"
                >
                  <option value="">{t('bulk_pick_courier')}</option>
                  {couriers.map((c) => (
                    <option key={c.id} value={c.id}>
            ...
            {selectionMode === 'verify' && (
              <>
                <p className="text-xs font-semibold uppercase tracking-wider text-[var(--color-text-faint)] mb-2">{t('bulk_verify_title')}</p>
                <button
                  onClick={handleVerify}
                  disabled={busy}
                ...
                {!showDenyInput ? (
                  <button
                    onClick={() => setShowDenyInput(true)}
                    disabled={busy}
                    className="w-full border border-red-500/50 text-red-600 dark:text-red-300 hover:bg-red-500/10 text-sm font-semibold py-3 rounded-xl transition-colors"
                  >
                    {t('bulk_deny')}
                  </button>
                ) : (
                  <div className="flex flex-col gap-2">
                    ...
                    <button
                      onClick={handleDeny}
                      disabled={busy || !denyReason.trim()}
                      className="bg-red-600 hover:bg-red-700 disabled:opacity-50 text-white text-sm font-semibold py-2.5 rounded-xl transition-colors"
                    >
                      {busy ? '…' : t('bulk_deny')}
                    </button>
                    <button
                      onClick={() => { setShowDenyInput(false); setDenyReason('') }}
                      className="text-xs text-[var(--color-text-muted)] hover:text-[var(--color-text-strong)]"
                    >
                      {t('btn_cancel')}
                    </button>
                  </div>
